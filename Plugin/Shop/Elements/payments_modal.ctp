  ...
  <div class="modal-dialog" id="boutique">
    <div class="modal-content">
      ...
      <div class="card-body">
        <?php if($isConnected AND $Permissions->can('CREDIT_ACCOUNT')) { ?>
          <?php if(!empty($starpass_offers)) { ?>
            <a class="btn btn-info btn-block" style="background-color:#1e6bcc" data-toggle="collapse" href="#starpass" aria-expanded="false" aria-controls="starpass">StarPass</a>
            <br>
            <div class="collapse" id="starpass">
                <form method="POST" action="<?= $this->Html->url('/shop/payment/starpass') ?>">
                  ...
                  <button type="submit" class="btn-shop"><?= $Lang->get('GLOBAL__SUBMIT') ?></button>
                </form>
                <br>
            </div>
          <?php } ?>
          <?php if(!empty($dedipass)) { ?>
            <a class="btn btn-info btn-block" style="background-color:#1e6bcc" href="<?= $this->Html->url(array('controller' => 'payment', 'action' => 'dedipass')) ?>">Dedipass</a><br>
          <?php } ?>
          <?php if(!empty($paypal_offers)) { ?>
            <a class="btn btn-info btn-block" style="background-color:#1e6bcc" data-toggle="collapse" href="#PayPal" aria-expanded="false" aria-controls="PayPal">PayPal</a>
            <br>
          ...
          <?php if($paysafecard_enabled) { ?>
            <a class="btn btn-info btn-block" style="background-color:#1e6bcc" data-toggle="collapse" href="#PaySafeCard" aria-expanded="false" aria-controls="PaySafeCard">PaySafeCard</a>
            <br>
            <div class="collapse" id="PaySafeCard">
              <form class="form-inline" data-ajax="true" action="<?= $this->Html->url(array('controller' => 'payment', 'action' => 'paysafecard')) ?>">
                ...
                <button type="submit" class="btn-shop pull-right"  style="margin-left: 10px;"><?= $Lang->get('GLOBAL__SUBMIT') ?></button>
              </form>
            </div>
          <?php } ?>
      ...
      <div class="card-footer">
        <button type="button" class="btn-shop" data-dismiss="modal" style="background-color:#1e6bcc"><?= $Lang->get('GLOBAL__CLOSE') ?></button>
        </form>
      </div>
    </div>
  </div>
